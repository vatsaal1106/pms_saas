...
class TimeLogsDataTable extends BaseDataTable
{
    ...
    public function query(ProjectTimeLog $model)
    {
        ...
        $request->endDate !== null && $request->endDate != 'null' && $request->endDate != '') {
            ...
            $startDate = Carbon::parse($request->startDate)->startOfDay(); // Assuming $startDate is already in a specific timezone
            $endDate = Carbon::parse($request->endDate)->endOfDay(); // Assuming $endDate is already in a specific timezone
