    ...
    public function dataTable($query)
    {
        ...
        $taskBoardColumns = TaskboardColumn::orderBy('priority')->get();
            ...
            if ($projectId != null && $projectId != 'all' && $projectId != 0) {
                ...
                $incompleteMilestones = collect(); // Return an empty collection if no valid projectId is provided
            }
        ...
        $datatables->editColumn('heading', function ($row) {
            ...
            if (!is_null($row->project_id) && !is_null($row->id) && in_array('projects', user_modules())) {
