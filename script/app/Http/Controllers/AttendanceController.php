...
class AttendanceController extends AccountBaseController
{
    ...
    public function show($id)
    {
        ...
        $attendanceSettings = EmployeeShiftSchedule::with('shift')->where('user_id', $attendance->user_id)->where('date', $attendance->clock_in_time)->first();
    ...
    public function bulkMark(StoreBulkAttendance $request) 
    {
        ...
        if ($request->mark_attendance_by == 'month') {
            ...
            $startDate = Carbon::parse(trim($dates[0]), $this->company->timezone);
            $endDate = Carbon::parse(trim($dates[1]), $this->company->timezone);
        }
